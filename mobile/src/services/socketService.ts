...
class SocketService {
  ...
  private async createConnection(): Promise<Socket | null> {
    try {
      // Kiểm tra kết nối API trước khi kết nối socket
      await testAndSetAPIConnection();
      ...
      // Lấy socket URL hiện tại từ hàm thay vì sử dụng hằng số
      const socketURL = getSocketURL();
      console.log("[SOCKET DEBUG] Connecting to socket at:", socketURL);
      ...
      this.socket.on("connect", () => {
        console.log("[SOCKET DEBUG] Socket connected with ID:", this.socket?.id);
  ...
  async hideMessage(messageId: string): Promise<boolean> {
    ...
    console.log(`Hiding message ${messageId} for current user only`);
    this.socket.emit("hideMessage", { messageId });
    return true;
  }
